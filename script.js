               ...
               (async() => {
                 ...
                 if(tx.signer.address.address === address.address) {  // 送信アドレスとウォレットのアドレスが同じかどうかで絵文字の表示と色を変える
                     if ([mosaicNames][0][0].names.length !==0){  // ネームスペースがある場合
                         ...
                         dom_amount.innerHTML = `<font color="#FF0000">💁‍♀️➡️💰 :<big><big><strong><strong> ${(parseInt(tx.mosaics[i].amount.toHex(), 16)/(10**div)).toLocaleString(undefined, { maximumFractionDigits: 6 })} </big></big></strong></strong></font>`;     // 　数量               
                 }else {
                     if ([mosaicNames][0][0].names.length !==0){ // ネームスペースがある場合
                         ...
                         dom_amount.innerHTML = `<font color="#008000">💰➡️😊 :<big><big><strong><strong> ${(parseInt(tx.mosaics[i].amount.toHex(), 16)/(10**div)).toLocaleString(undefined, { maximumFractionDigits: 6 })} </big></big></strong></strong></font>`;     // 　数量            
                 }
